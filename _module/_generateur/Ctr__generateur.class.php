...
class Ctr__generateur extends Ctr_controleur{
    ...
    function a_magicAllTables() {
        echo "<h1><a href='/'>Voir le site</a></h1>";
        $sql = "show tables";
        $result = Entity::$link->query($sql);
    ...
    private function creerClassTable($nomTable) {
        ...
        $chaine = file_get_contents("../_module/_generateur/template/Xxx.class.txt");
        ...
        file_put_contents("../_classe/$nomClasse.class.php", $chaine);
    }

    private function creerClassControleur($nomTable) {
    	$nomClasse = ucfirst($nomTable);
    	$nomCle = substr($nomTable, 0, self::$lprefixe) . "_id";
        $chaine = file_get_contents("../_module/_generateur/template/Ctr_xxx.class.txt");
        $chaine = str_replace("[nomTable]", $nomTable, $chaine);
        $chaine = str_replace("[nomClasse]", $nomClasse, $chaine);
        $chaine = str_replace("[nomCle]", $nomCle, $chaine);
        @mkdir("../_module/$nomTable");
        file_put_contents("../_module/$nomTable/Ctr_$nomTable.class.php", $chaine);
    }

    private function creerVueIndex($nomTable) {
        $chaine = file_get_contents("../_module/_generateur/template/xxx_index_vue.txt");
        $chaine = str_replace("[nomTable]", $nomTable, $chaine);
        $nomCle = substr($nomTable, 0, self::$lprefixe)  . "_id";
        $chaine = str_replace("[nomCle]", $nomCle, $chaine);
        $thnc=$this->thListeChamp(Entity::getChamps($nomTable));
        $chaine = str_replace("[thListeChamps]", $thnc, $chaine);
        $tdlv=$this->tdListeValeur(Entity::getChamps($nomTable));
        $chaine = str_replace("[tdListeValeur]", $tdlv, $chaine);
        file_put_contents("../_module/" . $nomTable . "/vue_" . $nomTable . "_index.php", $chaine);
    }

    private function creerVueEdit($nomTable) {
		$nomCle = substr($nomTable, 0, self::$lprefixe)  . "_id";
        $chaine = file_get_contents("../_module/_generateur/template/xxx_edit_vue.txt");
        $chaine = str_replace("[listeChamps]", $this->genererFormulaire($nomTable), $chaine);
        $chaine = str_replace("[nomTable]", $nomTable, $chaine);
		$chaine = str_replace("[nomCle]", $nomCle, $chaine);
        file_put_contents("../_module/" . $nomTable . "/vue_" . $nomTable . "_edit.php", $chaine);
    }

    private function creerMenu() {
        $chaine = file_get_contents("../_module/_generateur/template/inc_menu.php");
        $sql = "show tables";
        $result = Entity::$link->query($sql);
        $menu = "";
        while ($row = $result->fetch_row()) {
            $s='<?=hlien("' . $row[0] . '","index")?>';
            $menu = $menu . "<li><a href='$s'>" . ucfirst($row[0]) . "</a></li>\n";
        }
        $chaine = str_replace("[menu]", $menu, $chaine);
        file_put_contents("../_include/inc_menu.php", $chaine);
    }

    private function genererFormulaire($nomTable) {
        $chaine = "";
        $nomCle = substr($nomTable, 0, self::$lprefixe) . "_id";
        $result = Entity::$link->query("SHOW COLUMNS FROM $nomTable");
        while ($row = $result->fetch_row()) {
            /*
            echo "<pre>";
            print_r($row);
